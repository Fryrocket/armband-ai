...
Fix Pack 3:
  Per-reading session_id is preferred over batch-level session_id when
  building the row passed to insert_reading. session_id is now a first-class
  column on ppg_readings.
"""
...
def _zstd_rotator(source: str, dest: str) -> None:
    ...
    if not zstd:
        _warn_compression(f"zstd not on PATH – rotated log left uncompressed ({plain}). Install: sudo apt install zstd")
        try:
        ...
        return
    try:
        subprocess.run([zstd, "-f", "-q", "-o", dest, source], check=True, capture_output=True)
        os.remove(source)
    except (OSError, subprocess.CalledProcessError) as e:
        _warn_compression(f"zstd compress failed ({e}); leaving uncompressed archive if possible")
        try:
...
def setup_logging(level: str = "INFO", log_file: str | None = None, *, max_bytes: int = DEFAULT_LOG_MAX_BYTES, backup_count: int = DEFAULT_LOG_BACKUP_COUNT, compression: str = DEFAULT_LOG_COMPRESSION) -> None:
    ...
    if log_file:
        ...
        fh = RotatingFileHandler(path, maxBytes=max(1024, int(max_bytes)), backupCount=max(1, int(backup_count)), encoding="utf-8")
        if method == "gzip":
...
class ArmbandLogger:
    def __init__(self, config: dict | None = None):
        ...
        self.client = mqtt.Client(mqtt.CallbackAPIVersion.VERSION2, client_id=self.mqtt_cfg["client_id"], clean_session=True)
        self.client.on_connect = self._on_connect
        self.client.on_message = self._on_message
        self.client.on_disconnect = self._on_disconnect
        if self.mqtt_cfg.get("username"):
            self.client.username_pw_set(self.mqtt_cfg["username"], self.mqtt_cfg.get("password") or None)
        self._running = False
        self._msg_count = 0
    ...
    def _on_connect(self, client, userdata, flags, reason_code, properties=None):
        if reason_code == 0 or str(reason_code) == "Success":
            ...
            log.info("Connected to %s:%s – subscribed to %s and %s", self.mqtt_cfg["broker"], self.mqtt_cfg["port"], topic, batch_topic)
        else:
    ...
    def _on_message(self, client, userdata, msg):
        ...
        try:
            ...
            self._msg_count += 1
            log.info("#%d id=%d  bpm=%s  filt940=%s  moving=%s  boot=%s", self._msg_count, row_id, data.get("bpm"), data.get("filt940"), data.get("moving"), data.get("boot"))
        except Exception:
    ...
    def _handle_ios_batch(self, msg):
        ...
        batch_session_id = data.get("session_id")  # fallback only
        inserted = 0
        duplicates = 0
        for r in readings:
            if not isinstance(r, dict):
                continue
            # Prefer per-reading session_id (authoritative); fall back to batch-level.
            session_id = r.get("session_id") or batch_session_id
            row = {
                "bpm": r.get("bpm"), "spo2": r.get("spo2"), "temp": r.get("temp"),
                "motion": r.get("motion"), "moving": r.get("moving"),
                "raw940": r.get("raw940"), "filt940": r.get("filt940"),
                "batt": r.get("batt"), "trans": r.get("trans"), "ts": r.get("ts"),
                "id": r.get("id"),
                "source": data.get("source", "ios"), "batch_id": batch_id,
...
def main() -> None:
    ...
    method = _normalize_compression(log_cfg.get("compression", log_cfg.get("compress", DEFAULT_LOG_COMPRESSION)), log_cfg.get("compress", True))
    setup_logging(level=log_cfg.get("level", "INFO"), log_file=log_cfg.get("file"), max_bytes=int(log_cfg.get("max_bytes", DEFAULT_LOG_MAX_BYTES)), backup_count=int(log_cfg.get("backup_count", DEFAULT_LOG_BACKUP_COUNT)), compression=method)
    ArmbandLogger(cfg).start()
